...
- `components/nutrition/FoodLoggerSheet.tsx` — bottom sheet with tabs: **manual** entry, **search** (Open Food Facts + barcode), **saved** (favorites), and **AI** (dev-only). Tapping a search result or a favorite opens the **detail sheet** rather than logging inline; favoriting/logging flow through it. The favorites/search rows are the tappable `FoodRow` (icon · name · brand · category).
- `components/nutrition/FoodDetailSheet.tsx` — the Apple-style food detail sheet (glass shell + `useNav` nav-hide, same as FoodLoggerSheet). Header (emoji + ♥ favorite toggle + brand), 4 macro tiles, a segmented Protein/Carbs/Fat calorie-% bar (`macroCaloriePercents` in `lib/nutrition.ts`), a **collapsed "Nutrition details" disclosure** (only shown when a food has extended data) revealing the sugar/fiber/sat-fat/sodium breakdown + vitamins/minerals with %DV bars + Nutri-Score (official-scale colors) & NOVA badges, then quantity/portion (grams ↔ serving toggle) and an "Add to [meal]" CTA. Consumes a normalized per-100g `DetailFood`; scales macros + `detail` (`scaleDetail`) by the chosen portion on add. Reached from FoodLoggerSheet's search/favorites tabs (fixed meal) **and** the `/nutrition` favorites section (via the optional `mealSelector` prop — a meal-slot picker rendered above the CTA when the food isn't tied to a preset meal). The macro tiles + segmented bar + extended-details rendering come from the shared `NutritionFacts.tsx`.
...
- `lib/dailyValues.ts` — FDA `DAILY_VALUES` (in grams, unit-agnostic %DV), `percentDV()`, `formatNutrientAmount()` (g/mg/µg), and bilingual `NUTRIENT_LABELS`/`MICRO_KEYS` for the vitamins/minerals grid. `lib/foodIcons.ts` — `foodEmoji(category, name)` for the row/header icon.
