...
All shared TypeScript types live in `types/index.ts`: `Profile` (includes `gemini_api_key?: string | null`, the user's own AI key), `DailyWeightLog`, `WaterLog`, `FoodLog`, `SavedFood`, `ProgressPhoto`, `Exercise`, `WorkoutFolder`, `WorkoutSession`, `WorkoutSet`, `RoutineExercise`, `LoggedSet`, `Drop`, `SetType`, `WeightUnit`, `DistanceUnit`, `AppSettings` (the `app_settings` row shape), `AdminStats` (the `admin-stats` Edge Function response shape).
